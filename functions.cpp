...
void update_trading_state(const vector<HistoricalData>& historical_data, TradingState& state, float current_price, float thresholdExtraHigh) {
    ...
    // Print other volume conditions for transparency
    if (conditionHigh) {
...
void execute_trading_strategy(TradingState& state, float current_price, string& ans, int& amount) {
    // Check for buy signal: price crosses above HIGH
    bool buyCross = current_price > state.high;
    bool buySignal = buyCross && !state.lastSignalBuy;

    // Check for buy rule: price crosses above the high of the candle where buy signal was generated
    bool buyRule = false;
    if (buySignal) {
    ...
    // Check for sell signal: price crosses below stop-loss
    ...
    // Partial exit: Sell 50% of position at 2x stop-loss
